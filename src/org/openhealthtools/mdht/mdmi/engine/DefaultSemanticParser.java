...
public class DefaultSemanticParser implements ISemanticParser {
	...
	public void buildSemanticModel(MessageModel mdl, ISyntaxNode yroot, ElementValueSet eset) {
		...
		if (root != yroot.getNode()) {
			throw new MdmiException("Invalid root node, expected {0}, found {1}.", root.getName(), (yroot.getNode() == null ? "null" : yroot.getNode()
					.getName()));
		}
	...
	/**
	 * Set the value(s) and then the children recursively for the specified
	 * YNode from the given element value wrapper.
	 * 
	 * @param ynode
	 * @param xe
	 */
	 ...
	 *            The child type.
	 * @param index
	 *            The index of the child in the parent children of the same
	 *            type.
	 ...
	 *            path.
	 * @param node
	 *            The node relative to which we want the path (excluding the
	 *            node given).
	...
	/**
	 * Get the ynode to which the semantic element passed in is mapped to.
	 * Assumes the path is relative to the given ynode. Will create the ynodes
