...
public class DefaultSemanticParser implements ISemanticParser {
   ...
   private void getSimpleElement( YLeaf yleaf, XElementValue owner ) {
      ...
      try {
         ...
         throw new MdmiException("Error proccessing node " + DefaultSyntacticParser.getNodePath(yleaf.getNode()),
               throwable);
      }
   }
   ...
   private void getChoiceElement( YChoice ychoice, XElementValue owner ) {
      ...
      if( me == null ) {
         throw new IllegalArgumentException("Missing Semantic Elements for "
               + DefaultSyntacticParser.getNodePath(ychoice.getNode()));
      }
      XElementValue xe = new XElementValue(me, valueSet);
      if( owner != null ) {
         // set parent-child relationship
      ...
      try {
         ...
         throw new MdmiException("Error proccessing node " + DefaultSyntacticParser.getNodePath(ychoice.getNode()),
               throwable);
      }
   }
   ...
   private void getStructElement( YBag ybag, XElementValue owner ) {
      ...
      try {
         ...
         throw new MdmiException("Error proccessing node " + DefaultSyntacticParser.getNodePath(ybag.getNode()),
               throwable);
      }
   }
      ...
      if( node instanceof Bag ) {
         if( !(value instanceof XDataStruct) ) {
            throw new IllegalArgumentException("Invalid map: expected XDataStruct for node "
                  + ynode.getNode().getName() + " : " + DefaultSyntacticParser.getNodePath(ynode.getNode()));
         }
