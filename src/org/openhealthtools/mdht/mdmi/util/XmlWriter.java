...
public final class XmlWriter {
    ...
    * @param normalize Whether or not to normalize TEXT_NODES, etc.
    * 
    * @return The string representation of the DOM document.
    */
    ...
    * @param indent Indent to use (no of spaces, must be between 0 and 8).
    * @param maxLineLen Maximum line length to use (40 to 255).
    */
   ...
   public void write( Document doc, int indent, int maxLineLen, boolean normalize ) {
      ...
      m_indent = (indent < 0 || 8 < indent) ? INDENT : indent;
      m_lineLen = (maxLineLen < 40 || 255 < maxLineLen) ? LINE_LENGTH : maxLineLen;
      try {
   ...
   private void write( Node node, int indentLevel, boolean normalize ) throws IOException {
      ...
      case Node.TEXT_NODE: {
         ...
         if( v != null && v.length() > 0 ) {
            v = v.trim();
            if( v.length() > 0 )
               m_out.write(normalize ? normalize(v, false) : v);
         }
      }
         break;
   ...
   private void writeElement( Element node, int indentLevel, boolean normalize ) throws IOException {
      ...
      for( int i = 0; i < nAttrs; i++ ) {
         ...
         String sa = " " + attr.getNodeName() + "=\""
               + (normalize ? normalize(attr.getNodeValue(), true) : attr.getNodeValue()) + '"';
         aAttrs.add(sa);
         llen += sa.length();
         if( aLen < sa.length() )
            aLen = sa.length();
