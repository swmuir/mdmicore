...
public class XmlUtil {
   ...
   public static final Pattern           P_DATETIME   = Pattern.compile("-?([0-9]{4})-([0-9]{2})-([0-9]{2})T([0-9]{2}):([0-9]{2}):([0-9]{2})(.[0-9]*)?(Z|(\\+|-)([0-9]{2}):([0-9]{2}))?");

   // time
   // hh:mm:ss.sss(Z|+/-hh:mm)
   public static final Pattern           P_TIME       = Pattern.compile("([0-9]{2}):([0-9]{2}):([0-9]{2})(.[0-9]*)?(Z|(\\+|-)([0-9]{2}):([0-9]{2}))?");

   // date
   // (-)CCYY-MM-DD(Z|+/-hh:mm)
   public static final Pattern           P_DATE       = Pattern.compile("-?([0-9]{4})-([0-9]{2})-([0-9]{2})(Z|(\\+|-)([0-9]{2}):([0-9]{2}))?");

   // gYearMonth
   // (-)CCYY-MM(Z|+/-hh:mm)
   public static final Pattern           P_GYEARMONTH = Pattern.compile("-?([0-9]{4})-([0-9]{2})(Z|(\\+|-)([0-9]{2}):([0-9]{2}))?");
   ...
   // gMonthDay
   // --MM-DD(Z|+/-hh:mm)
   public static final Pattern           P_GMONTHDAY  = Pattern.compile("--([0-9]{2})-([0-9]{2})(Z|(\\+|-)([0-9]{2}):([0-9]{2}))?");
   ...
   public static Date parseDateYMDHMSMZ( String dateString ) throws ParseException {
      ...
      if( dateString.length() == 23 ) // tz missing
         dateString += "-0000";
   ...
   public static Date parseDateYMDHMSZ( String dateString ) throws ParseException {
      if( dateString == null || dateString.length() < 19 )
         throw new IllegalArgumentException("Invalid date string " + dateString);
      if( dateString.length() == 19 ) // tz missing
         dateString += "-0000";
   ...
   public static Date parseDateYMDZ( String dateString ) throws ParseException {
      if( dateString == null || dateString.length() < 10 )
         throw new IllegalArgumentException("Invalid date string " + dateString);
      if( dateString.length() == 10 ) // tz missing
         dateString += "-0000";
   ...
   public static Date parseDateHMSZ( String dateString ) throws ParseException {
      if( dateString == null || dateString.length() < 8 )
         throw new IllegalArgumentException("Invalid date string " + dateString);
      if( dateString.length() == 8 ) // tz missing
         dateString += "-0000";
    ...
    * 
    * @param root The element to add to.
   ...
   public static Element addElement( Element root, String name, String text ) {
      ...
      try {
         ...
         Element e = doc.createElement(name);
         root.appendChild(e);
         Text txt = doc.createTextNode(text == null ? "" : text);
         e.appendChild(txt);
         return e;
      }
      catch( Exception ex ) {
    ...
    * @param root The element to add to.
    * @param name The name of the new element.
    * @param before The child to insert before (may be null).
    * @return The newly created and added element.
    */
   public static Element addElement( Element root, String name, Element before ) {
      ...
      try {
         ...
         throw new MdmiException(ex, "XmlUtil.addElement({0}, {1}, {2}) fails", root.getNodeName(), name, before.getNodeName());
      }
   }
   ...
   public static Element addElement( Element root, String name, String text, Element before ) {
      ...
      try {
         ...
         throw new MdmiException(ex, "XmlUtil.addElement({0}, {1}, {2}, {3}) fails", root.getNodeName(), name, text, before.getNodeName());
      }
   }
    ...
    * 
    * @param doc Document to extract the fragment from.
    * @param path Path to the payload to extract, as described above.
    ...
    * The path starts relative to (and not including) the root (the document element).
    * 
    * @param mainDoc Header (envelope) document.
    * @param partDoc Payload (body) document.
    * @param path Path (position) where the payload document content is merged into the header.
